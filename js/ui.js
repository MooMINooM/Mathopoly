...
import { calculateRent, sellProperty, takeLoan } from './actions.js';
...
export function updateBoardUI() {
    state.boardSpaces.forEach(space => {
        if (space.type === 'property') {
            ...
            if (space.owner !== null) {
                spaceInfo.style.backgroundColor = state.players[space.owner].color;
                if (state.players[space.owner].color === 'var(--player2-color)' || state.players[space.owner].color === 'var(--player3-color)') {
